...
<span class="line"><span>VITE_API_SITE_URL=http://localhost:3000  # Site 模块地址</span></span></code></pre></div><h3 id="docker-部署" tabindex="-1">Docker 部署 <a class="header-anchor" href="#docker-部署" aria-label="Permalink to “Docker 部署”">​</a></h3><p>除了手动安装外，BBS-Go 也支持使用 Docker 和 Docker Compose 进行部署。项目根目录下的 <code>docker-compose.yml</code> 文件提供了完整的部署配置。</p><p>详细的 Docker 部署步骤请参考：<a href="./../docker.html">Docker 部署</a></p><h2 id="常见问题" tabindex="-1">常见问题 <a class="header-anchor" href="#常见问题" aria-label="Permalink to “常见问题”">​</a></h2><p>安装过程中可能遇到的常见问题：</p><ol><li>数据库连接问题：确保 MySQL 服务正常运行，且配置的用户名、密码和数据库名称正确。</li><li>端口占用问题：默认情况下，Server 使用 8082 端口，Site 使用 3000 端口，如果端口被占用需要修改配置。</li><li>文件上传问题：如果使用本地存储，确保配置的目录具有写入权限；如果使用阿里云 OSS，确保 AccessId 和 AccessSecret 配置正确。</li><li>跨域问题：在开发环境中，确保 Server 模块的 AllowedOrigins 配置正确。</li></ol><p>如果您在安装过程中遇到其他问题，请查看我们的问题反馈页面或在 GitHub 上提交 issue。</p>`,39)]))}const b=a(l,[["render",i]]);export{m as __pageData,b as default};
